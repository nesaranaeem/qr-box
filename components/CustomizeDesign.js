...
const CustomizeDesign = ({ qrOptions, setQrOptions }) => {
  const renderQRCode = () => {
    return (
      <QRCode
        ...
        level="L"
        renderAs="svg"
        imageSettings={qrOptions.logoImage ? {
  ...
  const handleLogoUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => setQrOptions({ ...qrOptions, logoImage: e.target.result })
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex items-center">
          ...
          {qrOptions.logoImage && <img src={qrOptions.logoImage} alt="Logo" className="ml-4 h-10 w-10 object-cover" />}
        </div>
      </div>
      <div className="mt-4">
